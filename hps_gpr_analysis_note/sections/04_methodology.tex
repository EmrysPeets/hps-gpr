...
The bin assignment is implemented with the histogram bin centers: a bin is part of
$\mathcal{B}_d(m_0)$ when its center lies inside the blind interval, and it is part of
the GP sideband training sample when its center lies outside the training-exclusion
interval. This center-based assignment is stable because the mass bins are narrower than
the detector resolution in the scanned range. The signal template itself is still
integrated over the full bin edges in Eq.~\eqref{eq:window-template}; only the choice of
which pre-existing histogram bins enter the blind window or sidebands uses the center
mask.
...
\end{equation}
with the baseline lower factor $k_{\min}=1.0$ for all three datasets. The practical
...
The lower-bound scan summarized in
Figure~\ref{fig:lengthscale-lower-bound-stability} documents the stability study that
motivated selecting the $1\sigma_x$ lower bound, i.e. the common $k_{\min}=1.0$
configuration used in the production fits. Panel (a) shows why the looser
$0.5\sigma_x$ option is unattractive: the optimized length scale can sit close to the
floor or vary irregularly with mass, which makes the fitted background too sensitive to
local statistical structure. The most compelling evidence is panel (b). The
$1.5\sigma_x$ option visibly over-constrains the background response, giving pull
widths far above the unit reference line, while the selected $1\sigma_x$ floor stays
near unit pull width across injected strengths without the instability of the looser
choice. This is why the $1\sigma_x$ lower bound is used as the nominal compromise
between flexibility and calibration.
...
The selected $1\sigma_x$ lower bound improves fit stability relative to the looser
$0.5\sigma_x$ option while avoiding the extra rigidity of the $1.5\sigma_x$
alternative. Panel (b), the average pull width versus injected significance, is the
primary selection diagnostic: the chosen curve remains close to the horizontal unit
reference line, while the more aggressive lower bound produces substantially
over-wide pulls.}
