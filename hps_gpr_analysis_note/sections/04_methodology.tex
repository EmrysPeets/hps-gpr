...
result is used to select the v4 baseline.
...
\end{equation}
The v4 configuration freezes the dataset-specific factors in
Table~\ref{tab:production-lengthscale-bounds}. It retains the reviewed lower bounds
from v3, while the 2021 upper factor is 15 in the present support study rather than
the factor 9 used in the earlier candidate.
...
\midrule
2015 full & 1.0 & 8 & frozen conservative baseline \\
2016 full & 0.9 & 8 & lower factor selected by prior closure \\
2021 10\% & 1.1 & 15 & endpoint-support stability setting \\
\bottomrule
\end{tabular}
\caption{Resolution-scaled RBF length-scale factors in the v4 configuration. The
factors multiply $\sigma_x(m)$ in
Eq.~\eqref{eq:dataset-lengthscale-bounds}. The labels summarize how each lower factor
or upper factor is used; they do not imply that one scalar ranking metric uniquely
determines all three choices.}
...
The upper factors remain 8 for 2015 and 2016. The 2021 factor is 15 in v4, following
the endpoint-support study that preceded this scan. These bounds keep the optimizer
