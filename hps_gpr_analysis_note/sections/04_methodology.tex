...
\end{equation}
with the baseline lower factor $k_{\min}=0.5$ for all three datasets. The practical
bookkeeping point is worth stating explicitly because an older diagnostic overlay
showed a stale 2016 value of 0.05; that lower-bound curve was incorrect and is not the
baseline used in the current production configs.
...
The current production configs retain one additional guard: a dataset-statistics floor
on $\ell_{\max}$ prevents the ceiling from becoming artificially small where the local
resolution is exceptionally tight. The older fixed-xrange cap remains available only as
an opt-in diagnostic knob in the code and is not enabled in the baseline results shown
here. A mass-independent lower floor is likewise a reasonable follow-up robustness
study, but it would constitute a deliberate method variation rather than the bound
prescription used for the results documented in this note.
...
$k_{\min}=0.5$ lower factor together with the dataset-specific upper factors. The
