...
\end{equation}
For the completed studies summarized in this note, the selected blind half-width is
$n_{\mathrm{blind}}=2.25$, as motivated by the validation studies in
Section~\ref{subsec:guard-band-construction}. For an ideal Gaussian line shape this
interval contains
\begin{equation}
...
\simeq 0.976,
\end{equation}
so only about 2.4\% of the signal lies outside the blinded region. This point matters
...
lets only the blinded subset enter the local likelihood. The same $2.25\,\sigma_d$
scale is used for the GP training exclusion in the production configuration, although
the package exposes the training exclusion and the extraction blind window as
...
configuration there is no additional guard gap between the extraction window and the GP
training sample: the training sample starts immediately outside
$\mathcal{B}_d(m_0)$. The validation campaign in
Section~\ref{subsec:guard-band-construction} compares the older $1.64\,\sigma_d$ blind
window plus a $2.25\,\sigma_d$ guard with the selected $2.25\,\sigma_d$
blind/extraction window. The latter keeps the same protection against near-tail signal
leakage in the training sidebands while retaining the $1.64$--$2.25\,\sigma_d$ signal
tails in the extraction likelihood. Full-refit pseudoexperiments in
Section~\ref{sec:toys} are the main guard against any residual signal-absorption bias
from this choice.
...
\end{equation}
with the baseline lower factor $k_{\min}=1.0$ for all three datasets. This
one-resolution lower bound is an additional stability choice: it prevents the optimizer
from using length scales shorter than the detector resolution while still allowing the
background model to follow detector-scale curvature.
...
stability choices anchored to detector resolution, not as arbitrary tune points. The
current production configs also retain a dataset-statistics floor on $\ell_{\max}$ so
the ceiling does not become artificially small where the local resolution is
exceptionally tight. The older fixed-xrange cap remains available only as an opt-in
diagnostic knob in the code and is not enabled in the baseline results shown here.
...
length-scale band obtained in the current implementation by applying the common
$k_{\min}=1.0$ lower factor together with the dataset-specific upper factors. The
purpose of the bound is to permit smooth background variation without allowing the
optimizer to drift into an unphysically unconstrained smoothing regime.}
\label{fig:lengthscale-bounds}
\end{figure}

The lower-bound scan summarized in Figure~\ref{fig:lengthscale-lower-bound-stability}
motivates the $1\sigma$ choice. Relative to a looser $0.5\sigma$ lower bound, the
$1\sigma$ floor stabilizes the optimized length-scale behavior and keeps the pull-width
response closer to the expected unit calibration without imposing the more rigid
$1.5\sigma$ alternative.

\begin{figure}[h!]
\centering
\begin{subfigure}[t]{0.88\linewidth}
  \centering
  \graphicorplaceholder{\linewidth}{kernel_lengthscale_figs/length_scale_diagnostics_by_lslb.png}
  \caption{Optimized length-scale diagnostics for candidate lower bounds.}
\end{subfigure}
...
\begin{subfigure}[t]{0.88\linewidth}
  \centering
  \graphicorplaceholder{\linewidth}{kernel_lengthscale_figs/overall_pull_width_vs_injsigma_by_lslb.png}
  \caption{Overall pull-width response versus injected significance.}
\end{subfigure}
\caption{Length-scale lower-bound stability comparison from the 2015 validation scan.
The selected $1\sigma$ lower bound improves fit stability relative to the looser
$0.5\sigma$ option while avoiding the extra rigidity of the $1.5\sigma$ alternative.}
\label{fig:lengthscale-lower-bound-stability}
\end{figure}
...
\begin{subfigure}[t]{0.45\linewidth}
  ...
  \caption{Sideband-only training and blinded-window prediction.}
\end{subfigure}
...
that enter the local likelihood.}
...
blind-window covariance. In practice, the Cholesky factor is the matrix that turns
independent unit-Gaussian nuisance coordinates into correlated bin-by-bin background
shifts with covariance $\bm{C}_d$. Introducing an auxiliary standard-normal nuisance vector
...
All expressions in this subsection use the selected $2.25\,\sigma_d$ blind-region
convention introduced in Section~\ref{subsec:guard-band-construction}.
...
\end{equation}
with a value near 0.976 for the selected $2.25\,\sigma_d$ half-width. The signal
...
\end{equation}
with $n_{\mathrm{blind}}=2.25$ in the current 95\% CL studies. This keeps the local
...
dataset has its largest \emph{local} upward fluctuation; any global overlays in those
auxiliary PNGs should be read as approximations rather than as finalized toy-calibrated
claims.
...
\begin{subfigure}[t]{0.32\linewidth}
  ...
  \caption{2021 1\%.}
\end{subfigure}
\caption{Local discovery p-value scans for the individual input datasets. The blue
curves are the one-sided local $p_0(m)$ values entering the fixed-mass discovery
interpretation. The global overlays are effective-trials diagnostics and are shown only
to indicate the approximate local threshold required for a scan-level statement.}
...
\end{subfigure}
\caption{Analytic significance curves for the corrected three-dataset combination. When
a full scan-level toy calibration of $q_{0,\max}$ is unavailable, the global curve
shown here is the effective-trials approximation described by
...
factor applied in the 90\% CLs cross-check files. The factor is unity below
