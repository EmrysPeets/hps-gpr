...
a per-hypothesis GP fit over one fixed, dataset-specific support and excludes only a
narrow mass-dependent training region around the test mass. The fit support is wider
than the interval searched for a signal, but it need not extend through empty turn-on
bins or the entire ROOT-histogram axis. That choice yields a background description
that is smoother, easier to validate, and naturally compatible with a combined
likelihood in a shared parameter of interest.
...
data beyond the search. Version 4 therefore keeps the established 19--90, 39--180,
and 50--250~MeV search intervals, while using 14--135, 30--210, and 40--300~MeV fit
supports for 2015, 2016, and 2021, respectively.
...
training bins at their upper endpoints. The corresponding 2021 counts are 9 and 55.
...
The active HPS configuration instead has no regional multiplier
(\texttt{pre\_alpha\_first\_n=0}) and fixes Eq.~\eqref{eq:alpha-oney} as the
delta-method variance model. This choice matters because \texttt{scikit-learn}'s
\texttt{alpha} has both an observation-noise interpretation and a numerical
...
result is used to qualify the v4.2 result.
...
favorable observed limit or local $p_0$. Only after those procedural checks and
coverage qualification may the range be treated as part of a physics analysis
card.
...
\subsection{2021 source-and-exposure hyperparameter pilot design}
\label{sec:planned-2021-exposure-ls-toys}

The 2021 pilot changes the expected count intensity, never a realized histogram or an
existing limit. Let $\lambda_i^{(1)}$ and $\lambda_i^{(10)}$ denote independently
constructed smooth expected intensities derived from the native 2021 1\% and 10\%
sources on the common 40--300~MeV support. The five declared scenarios are
\begin{align}
...
\end{align}
Within each source family, paired toys are generated with independent Poisson
increments,
\begin{align}
N_i^{1\%} &\sim \operatorname{Pois}\!\left(\lambda_i^{(1)}\right), &
N_i^{1\%\times10} &= N_i^{1\%}
 + \operatorname{Pois}\!\left(9\lambda_i^{(1)}\right), \nonumber\\
N_i^{1\%\times100} &= N_i^{1\%\times10}
 + \operatorname{Pois}\!\left(90\lambda_i^{(1)}\right), \nonumber\\
N_i^{10\%} &\sim \operatorname{Pois}\!\left(\lambda_i^{(10)}\right), &
N_i^{10\%\times10} &= N_i^{10\%}
 + \operatorname{Pois}\!\left(9\lambda_i^{(10)}\right).
\label{eq:v4p1-ls-pilot-increments}
\end{align}
Every marginal is therefore a Poisson draw from its declared mean, while the paired
comparisons share their lower-exposure realization
\cite{Kingman1993PoissonProcesses,HistFactory}.

The native 10\% source has 11.296 times the native 1\% support total, not exactly ten
times. The $1\%\times10$ versus native-10\% comparison is consequently a
source/selection-shape diagnostic, including the different native normalization, rather
than a pure tenfold-exposure closure test. The primary lane uses the same
\texttt{fGenGammaThresh} analytic family for both native sources; the independently
declared \texttt{fSigPowExpQ} lane is an alternate smooth-truth stress test. Neither
family is asserted to be the physical trident generator. Testing a second smooth
family probes background-shape dependence, but it does not implement the
discrete-profiled function-choice nuisance of Ref.~\cite{Dauncey2015DiscreteProfiling}.

Every pseudo-spectrum is passed through a fresh GP sideband fit with the production
40--300~MeV support, 50--250~MeV search interval, rebin-five geometry, and
$2.25\,\sigma_m$ training exclusion. The ten-toy pilot scans
$k_{\max}=6,9,12,15,20,$ and 25 on the predeclared
\SIlist{50;70;90;110;130;150;170;190;210;230;250}{MeV} grid. Per fit it records
$\ell_{\mathrm{opt}}/\sigma_x$, $\ell_{\mathrm{opt}}/\ell_{\max}$, boundary
occupancy, log marginal likelihood, optimizer status, count-space amplitude
diagnostics, and repeat stability. Questionable optimizer branches must be rerun with
the toy and card unchanged; they must not be repaired by interpolation.

The signal-response screen uses fixed absolute amplitudes at the predeclared
\SIlist{60;120;220}{MeV} injection masses. For each truth, scenario, toy, and mass,
the factor-15 background-only prefit-Asimov calibration defines
$\sigma_{A,\mathrm{ref}}^{(15)}$, and the predeclared levels
$z\in\{0,1,3,5\}$ define
\begin{equation}
A_{\mathrm{inj}}(z)=z\,\sigma_{A,\mathrm{ref}}^{(15)} .
\label{eq:v4p1-fixed-amplitude-anchor}
\end{equation}
The same numerical $A_{\mathrm{inj}}$ is then reused at every tested upper factor;
otherwise an apparent factor dependence could be produced simply by moving the
injection target. This is an Asimov calibration of the local response axis in the
sense of Ref.~\cite{Cowan2011}, not an expected-limit construction. At fixed
$A_{\mathrm{inj}}$, an increase in $\sigma_A$ indicates degraded local extraction
sensitivity, a displacement of $\hat A-A_{\mathrm{inj}}$ diagnoses signed bias, and
a response $(\hat A_z-\hat A_0)/A_{\mathrm{inj}}$ below unity diagnoses signal
absorption. Pull widths test the associated uncertainty calibration. With ten toys
these quantities are implementation screens, not precise ensemble statements.

This is a screening pilot in fit space. The immutable fit path used for the scan
predates the v4 uncropped fractional-bin density conversion, so its exported
$\eps^2$ values are explicitly non-promotable. The pilot produces no expected-limit
bands and does not establish \CLs{} coverage or a scan-wise global discovery
$p$-value. A larger production-matched closure ensemble, including signal-injection
bias and direct post-freeze coverage studies, remains necessary before a candidate
upper range can become a physics card. The pilot implementation and its deliberately
limited interpretation are documented in
Section~\ref{sec:v4p1-ls-exposure-pilot}.
...
It is shown alongside the local $p$-values in Section~\ref{sec:results}, but it is not
called a toy-calibrated global discovery probability. A background-only ensemble of
