...
\subsection{Cross-Kernel Closure Study}

Bias of the RBF kernel was tested against the $Matern32$ kernel via a cross-kernel pull-statistic study. First, a GP using the RBF kernel fits the 2016 10$\%$ dataset and 300 background-only toy distributions are thrown using the fitted model. This is repeated for the $Matern32$ kernel; 300 toy distributions using a model fitted by a $Matern32$ kernel are also generated. Next, the toys generated by the RBF kernel are fitted by the $Matern32$ kernel and vice versa. Varying amounts of signal are injected in a Gaussian shape at several mass hypotheses in the toys. Pull statistics are calculated.