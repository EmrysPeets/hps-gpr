...
Figure~\ref{fig:toy-upper-limit-proxy-funcform} puts the background-only rows of the same functional-form ensembles on the expected-limit scale used to compare scan sensitivity. The top panels keep the diagnostic in fitted signal-yield units, while the bottom panels divide by the per-mass $A(\eps^2=1)$ normalization so that the datasets can be read on a common coupling-squared scale. The shaded bands are toy-to-toy spreads of this simple proxy, not a systematic-uncertainty envelope. Within this subsection, the figure is therefore used as a compact smoothness and sensitivity cross-check: it would reveal mass-local instabilities in the functional-form closure samples, while the actual observed limits are still obtained from the profile-\CLs{} construction.
