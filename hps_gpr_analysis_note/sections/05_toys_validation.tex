...
In the comparison of different blind windows shown in ~\ref{fig:guard-band-reach-uncertainty}, train-$1.96$ gives the lowest median Gaussian-equivalent $\eps^2_{95}$ proxy, but it also leaves a much larger fraction of the injected signal template in the GP training region leading to higher leakage rate and lower signal sensitivity. The train-$2.25$ exclusion is the balanced candidate: it sharply reduces train-region leakag, is more sensitive to injected signal than $1.96$, and retains better expected reach than the wider $2.58$ and $3.0$ exclusions. The compact evidence is summarized in Table~\ref{tab:guard-band-choice-summary}; for the train-$2.25$ choice, the mean signal recovery is about $0.961$ at $Z_{\mathrm{inj}}=3$ and $0.943$ at $Z_{\mathrm{inj}}=5$, with a maximum injected training fraction of about $0.0258$.
...
The figures in this subsection are meant to be read sequentially. The construction cartoon in Figure~\ref{fig:blind225-cartoon} defines the selected widened blind region. Table~\ref{tab:guard-band-choice-summary} then gives the compact numerical trade-off. Figures~\ref{fig:guard-band-reach-uncertainty}--\ref{fig:guard-band-recovery-boxes} show why train-$2.25$ is a stable training-exclusion choice: narrower exclusions improve the expected-reach but leave too much signal in the training sidebands, while much wider exclusions throw away sideband information. Finally, Figures~\ref{fig:refmatched-blind-geometry-reach}- \ref{fig:refmatched-blind-geometry-deltaz} compare the two blind-window geometries once train-$2.25$ is fixed.
...
The fit-selection workflow scans a small menu of smooth positive candidate families, fits each family over the dataset-specific fit interval, evaluates bin-integrated goodness-of-fit over the scan-support region, and keeps only fits with stable parameters and visually acceptable residuals. The selected primary seeds are then frozen before the toy ensembles are generated. In the current campaign, the selected 2015 \texttt{fShiftSigPowTail} seed has Pearson $\chi^2/\mathrm{ndof}\simeq1.90$, the 2016 10\% \texttt{fShiftSigPowTail} seed has $\chi^2/\mathrm{ndof}\simeq0.99$, and the 2021 1\% seed uses a different \texttt{fSigPowExpQ} family with Pearson $\chi^2/\mathrm{ndof}\simeq1.70$.

\begin{figure}[H]
\centering
\graphicorplaceholder{0.96\linewidth}{toy_generation_figs/funcform_parameterization_summary.png}
\caption{Functional-form parameter panel for the selected toy seeds. Each row lists the primary analytic family, fit and scan intervals, bin-integrated goodness-of-fit metrics, and fitted parameter values used to generate the corresponding functional-form ensemble. The 2021 row uses a different primary family from the 2015 and 2016 rows. The ROOT $\chi^2/\mathrm{ndof}=-1.00$ entry in that row is an unavailable ROOT-side diagnostic placeholder; the Pearson and Neyman values are the ranking diagnostics used here.}
\label{fig:funcform-parameterization-summary}
\end{figure}

Figure~\ref{fig:analytic-fit-publication-overview} displays fits of candidate functional forms (left) and the mean generated value of each pseudoexperiment ensemble (right). The toy workflow keeps three mass intervals explicit in every export: the full histogram support used for toy normalization, the fit interval used to determine the analytic parameters, and the scan interval passed to the search code. Pseudo-data are generated bin-by-bin from Poisson means equal to the bin integrals of the selected analytic fit after rescaling that fit to the observed event count on the full support range. It also leaves the blind-window slice un-renormalized: the search interval inherits the normalization implied by the full histogram rather than by an isolated scan-window fit. The 2015 study keeps the scan hypotheses at \SIrange{20}{130}{MeV}; the 2016 toy-validation pass is now displayed with the updated \SIrange{42}{210}{MeV} scan window to suppress the low-mass mismodeling that biased the earlier 35--42~MeV region; and the 2021 1\% study uses its own fit family and scan support. The right-hand panels in Figure~\ref{fig:analytic-fit-publication-overview} show the most direct visualization of the corrected generation rule: the toy mean follows the selected fit while the toy totals fluctuate around, rather than being pinned to, the observed support-range event count.
...
Figures~\ref{fig:closure-validation-2015-funcform}--\ref{fig:closure-validation-2021-funcform} put the three dataset-specific functional-form closure suites on the same footing:  Each figure uses the full-refit toy rows for the selected $2.25\,\sigma_m$ blind/train geometry, groups the toys by mass and injected significance, and displays the pull mean, pull width, recovered amplitude, and residual significance for $Z_{\mathrm{inj}}=0,1,3,5$. The 2015 suite uses the \texttt{wide\_refmatched} subset of the blind-\(2.25\,\sigma_m\) refmatch comparison suite, the 2016 suite uses the \texttt{2016\_gpr/closure\_final} full-refit toys, and the 2021 suite uses the finalized \texttt{closure\_1pct/injection\_flat} \texttt{fSigPowExpQ} toys.
