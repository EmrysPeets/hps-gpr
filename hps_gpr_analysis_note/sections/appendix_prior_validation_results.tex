...
older plots the status of v4.1 result figures. In particular, the closure panels below
predate the selected year-dependent lower-length-scale factors, while the limit figures
predate the July 8, 2026 v3 configuration or use plotting repairs that had not yet been
replaced by rerun likelihood outputs. They remain useful for method auditing, but they
must not be combined numerically with the v4.1 candidate result. Each subsection records the
question asked by the study, the relevant configuration, and the conclusion that was
carried forward. Captions deliberately retain the historical confidence level and toy
convention, even where these differ from the present 90\% CL analysis.
...
not implement the three-dataset choices selected for the later v3 review candidate.
The error-barred, matched-reference lower-bound scans in
Figures~\ref{fig:v3-lslb-closure-2015}--\ref{fig:v3-lslb-closure-2021} supersede
them within the v3 validation chain. The functional-form seed and toy-mean overview
is shown where those pseudoexperiments are introduced in
...
$k_{\min}=(1.0,0.9,1.1)$ configuration. They were produced by rescaling the then-current
...
and should not be read as an expected sensitivity or observed exclusion. A later v3
projection used the same planning convention; it remains part of the archived v3
bundle and is not a v4 result.}
...
bundled with the archived v3 figure provenance. Version 4 does not reuse their
...
workflow. A corrected scan superseded it for the v3 review, but neither curve is a v4
result.}
...
curve remains in the archived v3 result bundle; the v4 discovery diagnostic is shown
separately in Section~\ref{sec:results-v4-local-p0}.}
...
signal-model cross-check rather than a uniform correction factor. The corrected 2021
result that superseded it remains in the archived v3 bundle and is not used as a v4
input.}
