...
This appendix compares the present GPR-based 2015 upper limit with the corrected
...
The current GPR method is not a polynomial re-fit with a smoother background curve.
...
hypothesis.  The 95\% upper limit is obtained with the bounded profile-likelihood
\CLs{} construction described in Section~\ref{sec:methodology}.

For the observed-limit comparison shown here, the GPR curve corresponds to the current
2015 production configuration with a widened $2.25\,\sigma_m$ blind and training
...
diagnostics.  The methodology text and final configuration should use a consistent
statement of this choice when this appendix is finalized.
...
\toprule
Feature & Corrected 2015 polynomial re-analysis & Present GPR analysis \\
\midrule
Data set & Fully unblinded 2015 engineering-run spectrum over the published low-mass
range. & Same 2015 spectrum for the comparison; current scan extends beyond the
published range in production studies. \\
...
\begin{figure}[p]
\centering
\graphicorplaceholder{0.92\linewidth}{final_limit_projection_figs/individual_datasets/hps2015_95cl_observed_vs_internal_note.png}
...
labeled ``GPR approach'' is the present GPR result for the 2015 data set.  The lower
panel shows the ratio GPR/Legendre.}
...
absence of signal absorption.  Those questions are answered by the toy program:
...
therefore be presented as a cross-check of the observed sensitivity scale, while the
closure and coverage figures remain the central statistical validation of the GPR
method.
