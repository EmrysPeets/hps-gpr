...
which the note now uses the bundled `summary\_combined\_all\_rad\_penalty` suite. In
...
\end{subfigure}
\caption{Current three-way combined significance summary using the bundled
radiative-penalty products. The left panel tests how unusual the observed
...
background and overlays the extracted signal component for each dataset. The lower
panel maps the extracted signal templates into the common variable
$(m-m_0)/\sigma_m$ and shows how the dataset-level extractions sum to the combined
black curve. The bundled image predates the lower-panel display fix, so the slight
edge truncation in the black curve is a plotting artifact of the old interpolation
path; the code has been corrected in the current repository so that the next
data-access rerender uses the full template support.}
