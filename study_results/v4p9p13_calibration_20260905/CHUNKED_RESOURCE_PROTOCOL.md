# Combined-only memory scheduling allowance, version 1

`run_chunked_refinement_6gib.py` is a resource wrapper around the frozen
`run_chunked_refinement.py` and `CHUNKED_REFINEMENT_PROTOCOL.md`. It permits a
source-derived per-worker peak-array budget of at most 6 GiB for the all-three
...
sampling, collector or audit source.

This allowance addresses combined coordinates deferred by the original 4 GiB
memory guard. It is not a new statistical study. The coordinator supplies the
...
budget is 6 GiB; a smaller positive budget is permitted.
...
10 GiB. Every worker retains one BLAS thread and chunks of 128 spectra. The
...
the requested bound for the actually accepted backend. A bound above 6 GiB is
...
The default output namespace is `chunked_resource_v1`. Scheduling slices, deferred
...
resource_policy:
  ...
  max_worker_memory_gib: 6.0
  max_companion_workers: 1
  max_companion_memory_gib: 4.0
  aggregate_memory_limit_gib: 10.0
...
Run `python3 -B run_chunked_refinement_6gib.py --pure-check` from this directory
to write `qa/chunked_resource_contract_test.json`. Its 26 checks cover allowed
...
against this wrapper's `layout_marker`, enforce combined scope and the 6 GiB
