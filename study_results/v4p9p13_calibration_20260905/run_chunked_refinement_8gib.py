#!/usr/bin/env python3
"""Combined-only resource wrapper: unchanged chunked inference, at most 8 GiB."""
...
RESOURCE_TYPE, RESOURCE_VERSION = "combined_memory_override", 2
BASE_IDENTITIES = {
...
def validate_resource_limits(max_memory_gib, scope):
    if (isinstance(max_memory_gib, bool) or not isinstance(max_memory_gib, (int, float))
            or not math.isfinite(max_memory_gib) or not 0 < max_memory_gib <= 8.):
        raise ValueError("Resource wrapper requires a finite positive bound of at most 8 GiB")
...
def layout_marker(max_memory_gib):
    ...
    marker["source_hashes"].update({str(p.relative_to(ROOT)): ref.sha(p) for p in
        (Path(__file__).resolve(), HERE/"CHUNKED_RESOURCE8_PROTOCOL.md")})
    marker["resource_policy"] = dict(type=RESOURCE_TYPE, version=RESOURCE_VERSION,
        scope_key=COMBINED, max_worker_memory_gib=8., max_companion_workers=1,
        max_companion_memory_gib=4., aggregate_memory_limit_gib=12.,
...
def pure_checks():
    ...
    check("accept_eight_gib_combined", validate_resource_limits(8., COMBINED) == COMBINED)
    check("accept_four_gib_combined_alias", validate_resource_limits(4., "all") == COMBINED)
    for name, value in (("zero", 0.), ("negative", -1.), ("over_eight", 8.000001),
    ...
    check("original_execution_identity_preserved", all(marker[k] == v for k, v in base.layout_marker(8.).items()
          if k != "source_hashes"))
    check("thread_chunk_and_aggregate_limits", marker["chunk_size"] == 128 and marker["blas_threads"] == 1
          ...
          and marker["resource_policy"]["version"] == 2
          and marker["resource_policy"]["max_worker_memory_gib"] == 8.
          and marker["resource_policy"]["max_companion_memory_gib"] == 4.
          and marker["resource_policy"]["aggregate_memory_limit_gib"] == 12.
    ...
    destination = HERE/"qa/chunked_resource8_contract_test.json"
...
def verify_resource_qa(marker):
    path = HERE/"qa/chunked_resource8_contract_test.json"
...
def main():
    ...
    parser.add_argument("--max-memory-gib", type=float, default=8.)
    ...
    requested = args.output or Path("chunked_resource8_v1")/f"attempt{args.attempt}_batch{args.batch_index:03}"
    ...
    extra = [Path(__file__).resolve(), HERE/"CHUNKED_RESOURCE8_PROTOCOL.md",
