...
follow the published/internal engineering-run summaries. For 2021, the baseline entries
follow the full-analysis-note preselection
\cite[Sec.~4.8, Table~38 and App.~A]{HPS2021FullAnalysisNote}, while the prompt entries
record the changes used to construct the regenerated 1\% GPR input.}
...
\Pro no additional prompt-only trigger bit is applied by the GPR histogram. \\
...
\Pro events with $p_{\rm sum}<2.8~\mathrm{GeV}$ are rejected; the selected input
therefore retains the high-$p_{\rm sum}$ side. \\
...
\Pro the regenerated sample requires $N_{\rm 2D~hits}(e^-)\ge8$; no additional change
to the positron hit requirement is asserted here. \\
...
\Pro no additional prompt timing requirement is documented beyond the production
preselection. \\
...
\Pro no additional prompt-tight offline coincidence value is documented for the GPR
histogram. \\
...
\Pro the regenerated 1\% input applies no vertex-fit $\chi^2$ requirement. \\

Positron cluster energy &
\Pre ---\newline \Pro --- &
\Pre ---\newline \Pro --- &
\Pre full-note sanity cut $E^{\rm clu}_{e^+}>0.2~\mathrm{GeV}$.\newline
\Pro no additional prompt positron-cluster threshold is documented beyond the
...
\Pro no separate prompt-tight cWAB requirement is documented for the GPR histogram. \\
...
\Pro no separate prompt-tight multiplicity requirement is documented for the GPR
histogram. \\
\end{longtable}
\endgroup
