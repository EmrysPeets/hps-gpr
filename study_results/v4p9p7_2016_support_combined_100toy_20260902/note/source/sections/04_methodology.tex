...
\subsection{From piecewise windows to fixed-support GP fits}

The original HPS GPR studies explored piecewise mass windows, redefined the sidebands
at each hypothesis, and considered several background profiles. They established that
a GP could interpolate the smooth trident spectrum, but the window-by-window fits also
produced less smooth limit curves, increased sensitivity to local fit boundaries, and
made a coherent multi-dataset combination difficult. The present workflow instead fits
the GP separately at each hypothesis on a fixed, dataset-specific support, excluding
only a narrow mass-dependent training region around the test mass. The support extends
beyond the signal-search interval, but need not include empty turn-on bins or the full
ROOT-histogram axis. This construction gives a smoother background prediction that can
be validated and used in a combined likelihood with a shared parameter of interest.

\subsection{Search interval and GP fit support}
\label{sec:fit-support-policy}

The search interval answers where a resonance hypothesis is tested. The GP fit support
answers which surrounding bins constrain the smooth background at that hypothesis.
Using the same endpoints for both definitions leaves the first and last test masses
with a one-sided training sample, even though the input histogram contains usable
data beyond the search. The accepted v4.2 three-campaign analysis uses the established
19--90, 39--180, and 50--250~MeV search intervals, with 14--135, 30--210, and
40--300~MeV fit supports for 2015, 2016, and 2021, respectively. The separate v4.9.5
2021-only pass keeps the 50--250~MeV search interval and changes that dataset's fit
support to 36--300~MeV.

These endpoints were declared before the v4 observed scan. The 2015 domain and the
upper 2016 edge follow the earlier functional-form validation fits. The 2016 lower
edge is placed at 30~MeV rather than the historical 35~MeV fit edge because the exact
rebin-five geometry matters: at a 39~MeV hypothesis the blind interval begins at
35.063~MeV, while the first bin center from a 35~MeV support is 35.125~MeV. A nominal
35--210~MeV support would therefore still contain no low-side training-bin center at
the lower search endpoint. The adopted domains retain at least 12 and 20 low-side
training bins at the 2015 and 2016 lower endpoints, and at least 138 and 51 high-side
training bins at their upper endpoints. For the accepted v4.2 geometry, the
corresponding 2021 counts are 9 and 55.
The bin-count audit is applied to every active dataset and mass hypothesis, rather
than inferred only from continuous support boundaries.

For 2021, v4.9 distinguished the support used to generate counts from the support
retained in a GP fit. Counts were generated once over 30--300~MeV; paired extractions
then used 30--300 and 40--300~MeV supports. This same-toy construction isolated the
effect of the lower edge, but did not freeze a new analysis support.

Version 4.9.5 tested lower edges at 32, 34, 36, 38, and 40~MeV, with 30~MeV retained
as an ineligible control, on the same 100 archived native-10\% conditional backgrounds.
The tests used masses of 55, 60, 65, and 70~MeV and injected strengths of
$z=0,2,5$. Under the original strict protocol, a candidate failed if any injected cell
had $|\overline{p}|\geq0.5$ and its two-sided 90\% Student-$t$ interval excluded zero;
no edge passed. The practical rule adopted after that phase required at least 9 of the
12 mass--strength means, including at least 3 of the 4 background-only means, to have
$|\overline{p}|<0.75$, with none at or above 1.25; the technical gates were unchanged.
Both 36 and 38~MeV qualified. The declared minimax tie rule selected 36~MeV because
its score lay within the 0.10 tie margin and it retained more support. The continuation
and the full 100-background summary passed without further retuning. This conditional
result froze the 36--300~MeV support for the v4.9.5 2021-only observed pass; it is not
a direct coverage result and does not revise the accepted v4.2 combined-analysis card.
The four configuration states are summarized in
Table~\ref{tab:v4p9-card-lineage}.

Version 4.9.7 is a later, separately frozen attempt to qualify the lower support for
full-2016 inference.  Its conditional Phase-1 ensemble rejects every eligible edge, so
the declared stop rule prevents a new 2016 observed scan and three-campaign
recombination.  Section~\ref{sec:v4p9p7-2016-support} gives that outcome; it is not a
retrospective alteration of any state in Table~\ref{tab:v4p9-card-lineage}.

\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{P{0.14\textwidth}P{0.20\textwidth}Y Y}
\toprule
Analysis state & 2021 mass domains & Fixed settings & Interpretation \\
\midrule
v4.2 analysis card & search 50--250; GP support:\newline 40--300~MeV &
Rebin five; log-mass/log-count preprocessing; $k_{\min}=1.1$,
$k_{\max}=15$; $2.25\sigma_m$ extraction and training exclusion; signed
amplitude and asymptotic 90\% \CLs{} &
Historical inference baseline.  Its accepted observed result is not changed or
recomputed by the support study. \\
v4.5 matched-refit contract & inherits the v4.2 domains &
Refit the GP for the matched background-only reference and every injected state;
define $\sigma_{A,\mathrm{ref}}$ from that reference; use
$z=0,1,3,5$ and retain signed pulls &
Defines the toy construction only; it neither changes the production settings nor
overrides the fit support. \\
v4.9 support diagnostic & search 50--250; generated spectra: \newline
30--300~MeV; paired GP supports: \newline 30--300 and 40--300~MeV &
The paired extractions use identical cached counts, 25 backgrounds per generating
source, and the same v4.2/v4.5 fit rules &
Completed conditional diagnostic. The lower-edge effect is isolated by a same-toy,
full-support control; no new production support is frozen. \\
v4.9.5 2021-only card & search 50--250; GP support: \newline 36--300~MeV &
1~MeV grid; $k_{\min}=1.1$, $k_{\max}=15$; $2.25\sigma_m$ extraction and training
exclusion; $1.64\sigma_m$ density window; signed amplitude; asymptotic 90\% \CLs{}
with $\alpha=0.10$ &
Frozen for the native-10\% 2021 observed pass after the declared continuation. The
support decision is conditional, is not direct coverage, and does not update the v4.2
three-campaign card. \\
\bottomrule
\end{tabularx}
\caption{Analysis states from v4.2 through v4.9.5. Search interval, toy-generating
support, and GP fit support are separate quantities. The accepted v4.2 combined result
and the v4.9.5 2021-only observed pass therefore retain distinct 2021 supports.}
...
\caption{Division of labor in the baseline GPR covariance. Panel (a) shows the
...
$e^{-1/2}\simeq0.61$. The length scale therefore changes the horizontal range over
which bins communicate. Panel (b) varies $C$ at fixed $\ell$. It changes the
...
changes one diagonal variance $\alpha_j$: a smaller value gives the highlighted bin
more leverage, while a larger value downweights it. HPS optimizes $C$ and $\ell$ on
the sidebands within configured bounds, but fixes $\alpha_i$ from the count-based
variance rule. All curves and observations are schematic and are not an HPS fit.}
\label{fig:gpr-hyperparameter-roles}
\end{figure}

Earlier studies explored more complicated kernels and piecewise procedures, but those
variants are no longer the production baseline. The Constant$\times$RBF model is the
analysis kernel; exploratory Constant$\times$Mat\'ern$(\nu=3/2)$ and piecewise-kernel
studies remain robustness checks. None provides a production-matched
alternative-kernel coverage result for the v4.2 analysis.
...
\qquad (\sigma_d \ll m),
\end{equation}
and uses this local physical scale to set the RBF bounds. Under the local,
resolution-scaled policy,
\begin{equation}
...
\end{equation}
Thus $\ell$ is a covariance-decay scale in the GP input coordinate: larger values
...
\begin{figure}[H]
\centering
\graphicorplaceholder{0.99\linewidth}{methodology_figs/gpr_mass_hypothesis_optimization.pdf}
...
solution. A larger data set also need not prefer a larger $\ell$: additional
...
The v4.2 result uses the dataset-specific factors in
Table~\ref{tab:production-lengthscale-bounds}. The lower factors retain their earlier
full-refit closure support; changing the 2016 upper factor does not requalify those
...
\begin{tabular}{lccc}
\toprule
Dataset & $k_{\min,d}$ & $k_{\max,d}$ & v4.2 status \\
\midrule
2015 full & 1.0 & 8 & unchanged from v4 \\
2016 full & 0.9 & 12 & accepted; conditional \\
2021 10\% & 1.1 & 15 & unchanged from v4 \\
\bottomrule
\end{tabular}
\caption{Resolution-scaled RBF length-scale factors in the v4.2 result. The
factors multiply $\sigma_x(m)$ in
Eq.~\eqref{eq:dataset-lengthscale-bounds}. Only the 2016 upper factor differs from
v4. Factor 12 is accepted as the current full-2016 setting, but the
observed-data selection remains distinct from a pre-unblinding freeze or direct
...
similar pull-width calibration and a weaker expected-reach proxy. The latter is
therefore retained as the conservative frozen baseline rather than described as the
numerical score winner. For 2016, the 25-toy continuation directly supports the
...
slightly smaller aggregate scores. The frozen 1.1 choice is therefore a
calibration--reach tradeoff, not a claim that it wins every diagnostic. These
development studies used the data fractions stated in Section~\ref{sec:toys}; they
should not be read as a new full-statistics validation of the v4.1 upper range.
...
accepted for the v4.2 result with an explicit post-selection qualification; it is
not presented as a pre-unblinding choice or a coverage-calibrated nominal.
Production-matched functional-form closure and direct coverage remain separate
requirements.
...
fixed-background fits, and the historical common-lower-bound overlays remain
robustness studies collected in the appendices.

An earlier 2021 source/exposure pilot used a nested-Poisson construction. It motivated
the later matched-refit studies but is not part of the v4.9 statistical definition.
...
\label{eq:aul-single}
\end{equation}
The v4 result uses $\alpha=0.10$, corresponding to a 90\% CL upper limit; retained
...
The v4.5 contract defines an injection axis through a background-only reference fit
that uses the same support, binning, exclusion mask, kernel bounds, preprocessing, and
GP-refit path as the corresponding toy extraction.  If
...
For accepted state $t$, the signed pull and paired response are
\begin{align}
...
Full refitting and no-refit displays answer different questions.  Holding a GP state
...
absorption.  Version 4.9 uses the latter matched-refit path.  Optimizer acceptance is
decided from fit state, likelihood, reproducibility, covariance, and kernel-coordinate
diagnostics before amplitudes or pulls are summarized; no row is selected because its
pull is favorable.
...
$\mathcal{O}(0.5)$. Their finite resolution is $1/N_{\mathrm{toys}}$. In the v4.2
combined ensemble, $N_{\mathrm{toys}}=300$, so a one-count change is
...
\end{equation}
with $\alpha=0.10$ for the v4 90\% CL result. The combined local significance is
evaluated from the same profiled likelihood
with the common signal vector $\eps^2 \bm{s}_{\mathrm{unit}}$. Inverse-variance
combinations are useful diagnostic proxies; they do not define the combined inference.
...
For the present result, Eq.~\eqref{eq:sidak-global} supplies a separate analytic
look-elsewhere reference for the local scan. The effective number of trials is
evaluated from the stated scan range and the mass-dependent resolution, so the
approximation accounts for the strong correlation between adjacent
\SI{1}{MeV}-spaced hypotheses rather than treating every scan point as independent.
It can be compared with the local $p$-value scan, but it is not a toy-calibrated global
discovery probability. A background-only ensemble of
the frozen search maximum, $q_{0,\max}$, remains the required calibration for any
future discovery-level claim; when that ensemble is available,
Eq.~\eqref{eq:pglobal-toy} supersedes the analytic approximation.
...
factor applied in the v4 90\% CL result obtained with \CLs{}. The factor is unity below
