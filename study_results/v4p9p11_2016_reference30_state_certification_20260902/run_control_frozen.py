#!/usr/bin/env python3
"""Execute the frozen v4.9.11 control, archive, and robust-state phases."""
...
def fixed_certificate(x_train: np.ndarray, y_train: np.ndarray, x_query: np.ndarray, mass: float, constant: float, length: float, recorded_lml: float | None, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    polish_pass = bool(
        polished.success and not caught and lml_improvement <= float(options["max_lml_improvement"])
...
def run_archive(spec: dict[str, Any]) -> None:
    require_frozen(CONTROL_FREEZE, "derived/control_adequacy/control_decision.json")
    control = json.loads((HERE / "derived/control_adequacy/control_decision.json").read_text(encoding="utf-8"))
    if control["status"] != "control_adequacy_pass" or control["script_sha256"] != sha256_file(Path(__file__)):
        raise StudyError("control phase is not frozen/passing or script drifted")
    values, edges = load_histogram(spec)
    centers, counts, _ = rebinned(values, edges, spec)
    ...
    for item in archived.itertuples(index=False):
        ...
        geometry_closure = bool(
            int(item.n_train) == int(train.sum())
            and math.isclose(float(item.ls_lo), expected_lower, rel_tol=2e-10, abs_tol=1e-12)
            and math.isclose(float(item.ls_hi), expected_upper, rel_tol=2e-10, abs_tol=1e-12)
        )
        ...
        base = {
            ...
            "archived_length_lower": float(item.ls_lo), "archived_length_upper": float(item.ls_hi),
            "archived_n_train": int(item.n_train), "geometry_closure_pass": geometry_closure,
...
def run_robust(spec: dict[str, Any], workers: int) -> None:
    ...
    for mass, group in attempts_frame.groupby("mass_GeV", sort=True):
        ...
        if selected is None:
            ...
            cert = fixed_certificate(centers[train], counts[train], centers[query], float(mass), selected.constant, selected.length, None, spec)
            selected_seed, constant, length, lml = selected.seed, selected.constant, selected.length, selected.lml
    ...
    for archived_row in archive_cert.sort_values("mass_GeV").itertuples(index=False):
        ...
        if len(match) == 1:
            ...
            row = {
                ...
                "warning_free_repeat_count": 0, "reproduced_warning_free_count": int(archived_row.reproducing_source_count),
...
def main() -> None:
    ...
    parser.add_argument("mode", choices=("control", "archive", "robust"))
