...
selection. The selection fixes the composition of the prompt $e^+e^-$ sample, while
the detector response sets the signal width and the radiative fraction sets its
normalization. This section describes those three ingredients together.
...
searches. For 2021, Table~\ref{tab:event-selection} distinguishes the upgraded-detector
preselection in the full analysis note from the v16 prompt-TC production used for the
native 10\% v4.9.5 source. The production record establishes
$p_{\mathrm{sum}}>\SI{2.8}{GeV}$ and no additional vertex-fit $\chi^2$ requirement.
It stores a three-dimensional hit count but does not establish the previously quoted
eight-hit two-dimensional electron requirement, so no such requirement is imposed in
this description.
...
The baseline 2021 entries follow the full analysis note
\cite[Sec.~4.8, Table~38 and App.~A]{HPS2021FullAnalysisNote}.
\ifwritingsample
The native 10\% spectrum comes from the v16 prompt-TC production. The GPR fit adds no
event-level cuts.
\else
Version 4.9.5 reads the native 10\% mass spectrum preselection/h\_invM\_8000 from a
...
and \emph{prompt selection} for the tighter resonance-search sample. The 2021 source is
closer to the latter because the v16 prompt-TC production retains the
high-$p_{\mathrm{sum}}$ side. Table~\ref{tab:event-selection} preserves that distinction
without inferring an unrecorded two-dimensional hit requirement.
...
$m_0=\SI{0.18}{GeV}$ with slope \num{0.0239} to avoid its high-mass turnover. The v4
2016 scan ends at \SI{180}{MeV}, close to the published \SIrange{39}{179}{MeV}
...
Figure~\ref{fig:mass-resolution-three-panel} compares the three implemented resolution
curves over their scan ranges. The preceding panels show the measurements underlying
each parameterization; the combined figure shows the smooth curves used to construct
the signal templates and excluded training windows.
...
configured high-mass continuation does not enter the v4 result. The 2021 panel shows the target-constrained V0 parameterization after
...
For the native 2021 10\% sample, the radiative-fraction input remains provisional. We
use
\begin{equation}
...
most of the 2021 search range, but it is not frozen for the final unblinded result.
That result will require a direct check of the normalization category against the v16
prompt-TC $p_{\mathrm{sum}}$ selection and its threshold convention, with the
mass-dependent parameterization retained as a cross-check.
...
The 2021 full analysis note derives a direct estimate from the generated RAD and WAB
cross-section distributions shown in
...
\end{equation}
The resulting one-sided 2021 penalty is 4.6\%, corresponding to a multiplicative
factor of 0.954. The 2015 and 2016 terms remain at 7\%.
