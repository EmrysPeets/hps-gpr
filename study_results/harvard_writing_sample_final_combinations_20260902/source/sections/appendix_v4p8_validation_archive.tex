\section{Archived Validation Batteries Through Version 4.8}
\label{app:v4p8-validation-archive}

\begin{center}
\fbox{%
\begin{minipage}{0.94\linewidth}
\textbf{Findings first.}  The v4.6 full-100 study confirmed that the
$1\%\times10$ 65~MeV offset persists beyond its ten-toy pilot.  The v4.7
threshold-specific truth reduced that offset but did not give uniform closure in
$1\%\times10$ and native 10\%.  The v4.8 residual-structured means failed their
source and signal-influence qualification gates; their completed extractions remain
conditional stress tests only.  These studies motivate v4.9 but do not contribute
rows to its 25-background ensemble and do not change the accepted v4.2 result.
\end{minipage}}
\end{center}

The detailed historical record follows so numerical provenance and negative results
remain reviewable.  It is intentionally outside the main decision path.  Repeated
method definitions should be read in light of the current definitions in
Section~\ref{sec:methodology}.

\subsection{Historical 2021 pilot definitions}

The source/exposure studies used independently fitted native 1\% and native 10\%
means, $\lambda_i^{(1)}$ and $\lambda_i^{(10)}$.  Their declared intensities were
\begin{align}
...
\end{align}
Within a source family the marginally Poisson samples were nested,
\begin{align}
N_i^{1\%}&\sim\operatorname{Pois}(\lambda_i^{(1)}), &
N_i^{1\%\times10}&=N_i^{1\%}+\operatorname{Pois}(9\lambda_i^{(1)}),\nonumber\\
N_i^{1\%\times100}&=N_i^{1\%\times10}+\operatorname{Pois}(90\lambda_i^{(1)}),
\nonumber\\
N_i^{10\%}&\sim\operatorname{Pois}(\lambda_i^{(10)}), &
N_i^{10\%\times10}&=N_i^{10\%}+\operatorname{Pois}(9\lambda_i^{(10)}).
\label{eq:v4p1-ls-pilot-increments}
\end{align}
The historical fixed-amplitude screen used
\begin{equation}
A_{\mathrm{inj}}(z)=z\,\sigma_{A,\mathrm{ref}}^{(15)},
\qquad z\in\{0,1,3,5\}.
\label{eq:v4p1-fixed-amplitude-anchor}
\end{equation}
These equations are retained here because the v4.5 archival subsection references
them.  They do not define the v4.9 source truth or pair the two native source families.
...
The complete ensemble confirms a local conditional 65~MeV offset in the
...
\paragraph{Finding summary.}
A source-dedicated turn-on truth improves the 65~MeV response for
$1\%\times10$ but leaves nonuniform offsets in both targeted lanes and neighboring
masses.  Better threshold modeling is relevant, but this 20-background study does
not freeze a support or generator choice.
