...
def summary_tail(samples,threshold):
    ...
    return dict(k=k,n=n,p=k/n,interval95=interval(k,n),upper95_one_sided=1. if k==n else float(beta.ppf(.95,k+1,n-k)))
def holm(p):
...
def main():
    ...
    args=ap.parse_args();dataset_label={'2015':'2015 full','2016':'2016 full','2021':'2021 10%'}[args.dataset];start=time.monotonic();folder=HERE/'global_fast'/args.dataset;out=folder/'analysis';out.mkdir(exist_ok=True)
    figdir=HERE/'figures';figdir.mkdir(exist_ok=True)
    from verify_acceleration import verify
    acceleration=verify(args.dataset)
    contracts=[]
    ...
    result=dict(dataset=args.dataset,dataset_label=dataset_label,scope='finite 1 MeV grid; common archived stress truth; frozen kernels',masses_MeV=masses.tolist(),gp_samples=args.gp_samples,methods={},acceleration_audit_passed=acceleration['passed'])
    ...
    for method in METHODS:
        ...
        for ordering,g,d in [('minimum_local_p',gp_max,direct_max),('maximum_raw_root',gp_raw,direct_raw)]:
            ks=ks_2samp(g,d,method='asymp')
            info[ordering+'_maximum_distribution_KS']={'statistic':float(ks.statistic),'pvalue':float(ks.pvalue),'test':'two-sample, marginal over complete scan maxima; four comparisons across extension datasets/methods per ordering'}
        ...
        maxima.update({method+'_gp':gp_max,method+'_direct':direct_max,method+'_gp_raw':gp_raw,method+'_direct_raw':direct_raw})
    ...
    result['input_sha256']={str(p.relative_to(HERE)):sha(p) for p in [folder/'asimov/scan_vectors.npz',folder/'validation1000/scan_vectors.npz',folder/'pilot10/summary.json',folder/'acceleration_validation.json',HERE/'verify_acceleration.py',Path(__file__),HERE/'PROTOCOL.md']}
    ...
    for ax,method in zip(axes,METHODS):
        ...
        ax.plot(masses,np.maximum(d.p_local_common_truth,1e-8),color=COLORS['local'],label='Local: common-background Gaussian')
        ax.plot(masses,np.where(d.p_global_gp>0,d.p_global_gp,np.nan),color=COLORS['global'],label=f'Global: GP field, {len(masses)} masses')
        ax.plot(masses,d.p_global_raw_ordering,color='#B87715',ls=':',label='Global: raw-root ordering')
        zero=d.p_global_gp==0
        if zero.any():ax.scatter(masses[zero],d.p_global_gp_high[zero],marker='v',color=COLORS['global'],label='Zero GP tail: upper 95% interval end',s=25)
    ...
    fig.suptitle(f'{dataset_label}: local and global probabilities under one common stress background',fontsize=12,y=.995)
    handles={}
    for ax in axes:
        hh,ll=ax.get_legend_handles_labels()
        handles.update(zip(ll,hh))
    fig.legend(handles.values(),handles.keys(),loc='upper center',bbox_to_anchor=(.5,.96),ncol=2,fontsize=9,frameon=False)
    ...
    for ax,method in zip(axes,METHODS):
        ...
        ax.plot(d.threshold,np.where(d.gp_p>0,d.gp_p,np.nan),label=f'{args.gp_samples:,} GP fields',color=COLORS['global'])
