#!/usr/bin/env python3
"""Run the v4.8 sparse-rigid functional-form injection validation.
...
v4.7.1 deterministic optimizer-repeat gate so that optimizer selection is based
only on likelihood and branch reproducibility, never on the fitted signal,
pull, recovery, or epsilon-squared coordinate.

The frozen source shape is fit once to native 1% and normalization-scaled into
native 10%; it is never refit to the high-statistics source or exposure lanes.
The required ROOT layout is

    toys/rigid_ggt26_scaled1pct/{scenario}/toy_NNNN

for the four requested exposure scenarios at toy indices 0--19.  The cached
indices 20--24 remain untouched reserve backgrounds.  All scientific inputs,
the frozen card, and the study-local runtime snapshot are verified against
SHA-256 declarations in ``rigid_study_spec.json`` before production.
"""
...
RUNS = HERE / "runs/rigid_closure_v4p8p2_20toy_frozen"
DERIVED = HERE / "derived/rigid_closure_v4p8p2_20toy_frozen"
QA = HERE / "qa/rigid_closure_v4p8p2_20toy_frozen"
...
def assert_spec_contract(spec: Mapping[str, Any]) -> None:
    ...
    if toy_indices != tuple(range(N_TOYS)):
        raise StudyError("toy_indices must be the predeclared contiguous range 0--19")
    ...
    if (
        str(rigid_payload.get("generator_tag")) != "rigid_ggt26_scaled1pct"
        ...
        or tuple(
            map(
                float,
                rigid_payload.get("primary_search_region_metrics_0p125mev_bins", {}).get(
                    "region_gev", ()
                ),
            )
        )
    ...
    if (
        int(product.get("n_toys_available", -1)) != N_AVAILABLE_TOYS
        or int(product.get("n_toys_analyzed", -1)) != N_TOYS
        or tuple(map(int, product.get("reserve_toy_indices", ())))
        != tuple(range(N_TOYS, N_AVAILABLE_TOYS))
    ):
        raise StudyError("available/analyzed/reserve toy partition drift")
...
def validate_toy_product(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    expected_manifest_keys = {
        ...
        for toy_index in range(N_AVAILABLE_TOYS)
    }
    ...
    for family in ("one_pct", "ten_pct"):
        ...
        with uproot.open(resolve_study_path(str(source_record["path"]))) as source_file:
    ...
    with uproot.open(path) as root_file:
        ...
        for scenario in all_scenarios:
            ...
            for toy_index in range(N_AVAILABLE_TOYS):
                ...
                outside_support = (centers < 0.04) | (centers >= 0.30)
                if np.any(rounded[outside_support] != 0):
                    raise StudyError(f"toy has nonzero counts outside 40--300 MeV: {key}")
                ...
                if (
                    ...
                    != stable_seed_words(
                        "nested_poisson", family, int(toy_index), stage
                    )
                ):
                ...
                if scenario in SCENARIOS:
                    ...
                    in_support = (rebinned_centers >= 0.04) & (
                        rebinned_centers < 0.30
                    )
        ...
        manifest_truths = {
            ...
            for row in manifest.get("truths", ())
        }
        ...
        for scenario in all_scenarios:
            ...
            if scenario in SCENARIOS and str(
                analytic_keys.get(scenario, "")
            ) != analytic_key:
    ...
    return {
        ...
        "bins_per_histogram": int(len(source_edges) - 1),
        "histogram_range_gev": [
            float(source_edges[0]), float(source_edges[-1])
        ],
        "gp_support_gev": [0.04, 0.30],
        "manifest_content_sha256": recorded_content_hash,
        "count_inventory_sha256": canonical_json_hash(count_hashes),
        "hidden_parent_histograms": N_AVAILABLE_TOYS,
        "reported_histograms": len(SCENARIOS) * N_AVAILABLE_TOYS,
        "analysis_histograms": len(SCENARIOS) * N_TOYS,
        "reserve_indices": list(range(N_TOYS, N_AVAILABLE_TOYS)),
...
def preflight(*, validate_inventory: bool = True) -> dict[str, Any]:
    ...
    require_hash(root_path, root_sha, "rigid-generator background toys")
        ...
        if isinstance(fit_path_value, str) and isinstance(fit_sha, str):
            ...
            for family in ("one_pct", "ten_pct"):
                ...
                if (
                    ...
                    raise StudyError(
                        f"selected rigid-generator fit fails fixed gates: {family}"
                    )
    ...
    if set(modules) != set(V4P6_COMPATIBILITY_RUNTIME_SHA256):
        raise StudyError("declared runtime-module set is incomplete or contains drift")
    manifest_value = runtime.get("package_manifest")
    manifest_sha = runtime.get("package_manifest_sha256")
    if not isinstance(manifest_value, str) or not isinstance(manifest_sha, str):
        raise StudyError("runtime package manifest path/hash is missing")
    runtime_manifest_path = resolve_study_path(manifest_value)
    require_hash(runtime_manifest_path, manifest_sha, "runtime package manifest")
    ...
    if (
        ...
        raise StudyError("runtime package manifest schema is invalid")
    actual_runtime_files = {
        str(path.relative_to(RUNTIME_ROOT)): sha256_file(path)
        for path in sorted((RUNTIME_ROOT / "hps_gpr").glob("*.py"))
    }
    if dict(declared_runtime_files) != actual_runtime_files:
        raise StudyError("runtime package file-set or content drift")
    checks["runtime_package_manifest"] = True
    for module_path_text, record in modules.items():
        ...
        archived_path = resolve_study_path(archived)
        require_hash(
            archived_path, expected, f"archived runtime {module_path_text}"
        )
        module_name = str(module_path_text).replace("/", ".")
        if module_name.endswith(".py"):
            module_name = module_name[:-3]
        imported = importlib.import_module(module_name)
        imported_path = Path(str(getattr(imported, "__file__", ""))).resolve()
        require_hash(imported_path, expected, f"imported runtime {module_path_text}")
        if RUNTIME_ROOT.resolve() not in imported_path.parents:
            raise StudyError(
                f"runtime overlay was not imported for {module_path_text}: {imported_path}"
            )
        ...
        if compatibility_hash is None or expected != compatibility_hash:
            raise StudyError(
                f"runtime is not hash-compatible with the declared integration state: {module_path_text}"
            )
    ...
    prediction_fields = set(getattr(BlindPrediction, "__dataclass_fields__", {}))
    ...
    if not required_prediction_fields.issubset(prediction_fields):
    ...
    if (
        ...
        or any(character not in "0123456789abcdef" for character in driver_hash)
    ):
        raise StudyError("production driver SHA-256 is missing or malformed")
    ...
    row.update(
        {
            ...
            "analysis_partition": "rigid_scaled1pct_20toy_validation",
...
def collect() -> dict[str, Any]:
    ...
    result = {
        ...
        "interpretation": (
            "Twenty-background rigid conditional injection-extraction "
