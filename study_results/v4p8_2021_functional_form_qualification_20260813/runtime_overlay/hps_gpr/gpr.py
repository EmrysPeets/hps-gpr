...
def fit_gpr(
    ...
    *,
    ...
    Args:
        ...
        random_state: Explicit optimizer-restart seed.  When omitted, the
            optional ``config.gp_optimizer_random_state`` value is used before
            falling back to scikit-learn's legacy global-RNG behavior.
    ...
    if random_state is None:
        configured_seed = getattr(config, "gp_optimizer_random_state", None)
        random_state = None if configured_seed is None else int(configured_seed)
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        fitted = gpr.fit(X_in.reshape(-1, 1), y_in)
    fitted._hps_optimizer_random_state = random_state
    fitted._hps_optimizer_warnings = tuple(
