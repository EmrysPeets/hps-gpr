...
normalization. The v5.0.5 note retains the search ranges 2015: 19--100~MeV,
2016: 39--180~MeV, and 2021: 50--250~MeV. Declare the local-GP
and archived-stress generating spectra.}}
\\[5pt]$\downarrow$\\[5pt]
\fbox{\parbox{0.88\linewidth}{\centering
...
only and S+B denotes signal plus background. The displayed protocol
can be applied over the new 2015 range; the archived two-truth calibration
results below retain their original tested masses. The 300-toy extension
bands in Appendix~\ref{app:v504-extension} do not by themselves execute
this two-truth calibration.}
...
assumed interpolation. The alternative stress background is a second smooth
...
construction or a known limitation. The resulting shape still needs to be
tested against data.
...
To sample rare tails efficiently, the study generated equal strata
...
\end{tabular}
\caption{Completed fits and Monte Carlo precision in the calibration study.
The final columns are median fixed/profiled observed-limit ratios at
matched masses. The precision listed here concerns the finite toy samples;
it does not measure the accuracy of the background model.}
...
localized and depends on the stress background. That background includes a
component from a fit that did not converge, which limits the interpretation
of this comparison.}
...
chosen background family; it does not establish which model describes the data.}
...
noisy estimate displayed at one; the saved numerical results retain unclipped values.
...
establish percent-level accuracy at every mass.
...
a result. The discrepancies in the independent 2016 parameter fits,
described in Appendix~\ref{sec:v5-historical-validation}, require a separate
