...
Each HPS dataset is represented by an invariant-mass spectrum formed after a
run-specific event selection. That selection determines the composition of the prompt $e^+e^-$ sample;
...
event-level cuts.
\else
The 10\% mass spectrum is the prompt-TC histogram
\texttt{preselection/h\_invM\_8000}. The GPR fit adds no event-level cuts.
\fi
Each campaign enters the analysis through its selected invariant-mass histogram and
the detector response and signal normalization for the same event category.

The engineering-run notes use \emph{preselection} for the broad reconstruction sample
and \emph{prompt selection} for the tighter resonance-search sample. The 2021 dataset is
...
\input{sections/v505_geometry_main}
\FloatBarrier
...
requirement is applied to the 2021 dataset used in the GPR fit. These archived plots
...
\begin{figure}[htbp]
  ...
  \caption{Reconstructed-vertex distributions after the loose preselection,
    ...
    the 2021 dataset used in the GPR fit.}
...
\includegraphics[width=0.94\linewidth]{../figures/v5_nominal_mass_resolutions.pdf}
\caption{Nominal mass-resolution parameterizations over the
search intervals. These functions set the signal-template widths, moving fit
windows and resolution-scaled kernel constraints. Their signal-width
variations are examined in Section~\ref{sec:v5-resolution-results}.}
\label{fig:v5-nominal-resolution}
\end{figure}
...
\begin{figure}[H]
\centering
\graphicorplaceholder{0.78\linewidth}{../figures/v504_mass_resolution_three_panel.pdf}
...
The effect of varying this nominal signal width is examined in
...
engineering-run normalizations follow the corresponding prompt searches, while the 2021
...
\caption{Determination of the 2015 radiative fraction. The left panel shows the selected
...
range. For the 2016 dataset, we instead use the constant approximation
\begin{equation}
...
the selected radiative, trident, and converted-WAB simulated samples
...
cross-section normalization for the 2021 radiative-fraction calculation.}
...
Together with the relative normalization $R=0.244$, these quantities give the 3.5\%
...
before applying it to the full dataset \cite[Sec.~9,
