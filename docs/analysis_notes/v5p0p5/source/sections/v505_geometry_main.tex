...
The v5.7.1 study follows prompt $A'\to e^+e^-$ decays through the sensor
geometries and field maps used for each campaign. The sensor shapes and
positions are read from Linear Collider Detector Description (LCDD) files,
the XML geometry files used by the detector simulation~\cite{McCormickLCDD2005}.
The daughter momenta
