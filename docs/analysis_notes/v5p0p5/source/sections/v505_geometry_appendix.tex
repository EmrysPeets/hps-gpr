...
The sensor geometry is read from the Linear Collider Detector Description
(LCDD) files in \texttt{JeffersonLab/hps-java}, revision
\texttt{473c732d}~\cite{v571:geometry-source}. These XML files specify the
active sensor shapes, positions and rotations within the detector.
...
\toprule
Run & Positron & Electron & Selection reference\\
\midrule
...
revision \texttt{feadfcbd}~\cite{v571:fieldmaps}. The 2015 calculation adopts
...
\begin{table}[H]
\centering\small
\input{geometry_tables/geometry_table.tex}
...
respectively, and ends at 300 MeV. The 2015 histogram ends at 150 MeV; its grey
