...
MEMORY="${MEMORY:-6G}"
CONFIG="${CONFIG:-study_configs/config_2015_2016_combined_extraction_display_v15p8.yaml}"
DATASET="${DATASET:-combined}"
BASE_OUTPUT="${BASE_OUTPUT:-outputs/extraction_display_2015_2016_combined_batch}"
LOGDIR="${LOGDIR:-logs/extraction_display_2015_2016_combined}"
...
for Z in "${SIGMA_LIST[@]}"; do
  sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=hps_exdisp_comb_z${Z}
